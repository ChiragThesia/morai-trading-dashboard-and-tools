...
| `eventAdjustment` | 5 | 1 − Σ(front-leg FOMC/CPI/NFP × 0.25), penalty ×2 when the event collides with the peak-theta window (final 5 days before front expiry — the forced pre-event exit forfeits the richest decay), floor 0. Softened 0.5→0.25 (2026-07-15): any 20–35 DTE front window spans ≥2 macro events, so the old penalty saturated the criterion to a permanent 0 | No binary catalysts inside the short leg (practitioner consensus); graded so the criterion discriminates instead of dying |
| `beVsEm` | 15 | breakeven width / (spot·σ_f·√(t_f/365)); fraction = clamp01(ratio/1.5) — reverted 2.0→1.5 (2026-07-15): live ATM-calendar candidates cluster ~1.1×, 2.0 was unreachable and compressed the criterion; <2 breakevens → 0 | Profit-zone width vs expected move (real bisection breakevens, D-09) |
| `deltaNeutral` | 15 | fraction = clamp01(1 − \|Δ_net\|/5) — tightened /10→/5 (user: "near 0 basically if possible") | User-locked 2026-07-08/09 |
| `debitFit` | 5 | haircut debit vs ideal $3.2k–5k: full credit in band, 0.7 floor at ≤$2k (cheap ok), 0 at ≥$7.5k (expensive fades) | User-locked spend preference (2026-07-09) |
| `thetaVega` | 10 | θ_net/vega_net; fraction = clamp01(ratio/0.20) — full credit AT the tastytrade practitioner floor ("vega no more than 5× theta"); aligned 0.25→0.20 (2026-07-15); vega 0 → 0 | Promoted from experimental 2026-07-09 (user lock; PICK-04 re-arbitrates); tastytrade/OptionsTradingIQ |
