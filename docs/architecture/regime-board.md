...
`t10y2y`/`t10y3m` are a CLIENT-VISUAL-ONLY display band: the rates come from `useMacro`/
`macro_observations`, not `useRegimeBoard` — there is no server-side regime indicator for
these two ids, and the picker gate stays BLIND to them (no new gate inputs), the same
"display only this phase" posture as the four admitted indicators above.
