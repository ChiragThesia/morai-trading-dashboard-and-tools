...
type StrikeGex = { k: number; gex: number };
...
type Snapshot = {
  ...
  putWall: number | null;
  netGamma: number;
  strikes: ReadonlyArray<StrikeGex>;
};

/** Widest a zone may sit from spot and still be drawn — mirrors gMaxPct in gamma-levels.pine. */
const ZONE_MAX_PCT = 4;
/** Zone cap. The study declares max_boxes_count = 30; staying under it keeps Pine from
 *  silently garbage-collecting the oldest boxes instead of raising an error. */
const ZONE_LIMIT = 24;

/**
 * jsonb arrives as already-parsed JS. Validate the shape rather than asserting it — a vendor
 * or migration change that alters the element shape must surface as "no zones", never as
 * NaN strikes drawn at price 0.
 */
function toStrikeGex(v: unknown): ReadonlyArray<StrikeGex> {
  ...
  for (const e of v) {
    ...
    const k = num(rec["k"]);
    const g = num(rec["gex"]);
    if (k !== null && g !== null && k > 0) out.push({ k, gex: g });
  }
  return out;
}

/** Nearest-to-spot band, then the biggest by magnitude, then back into strike order. */
function pickZones(strikes: ReadonlyArray<StrikeGex>, spot: number): ReadonlyArray<StrikeGex> {
  if (spot <= 0) return [];
  return strikes
    .filter((s) => Math.abs(s.k - spot) / spot * 100 <= ZONE_MAX_PCT)
    .toSorted((a, b) => Math.abs(b.gex) - Math.abs(a.gex))
    .slice(0, ZONE_LIMIT)
...
async function latestSnapshot(sql: SQL, lastGood: boolean): Promise<Snapshot | null> {
  ...
  // `strikes` follows the same coalesce as the walls above, and for the same reason: prefer
  // the near-term set, fall back to the all-expiry one when it is absent. Today it ALWAYS
  // falls back — nothing writes near_term.strikes yet — so the zones currently carry
  // all-expiry OI and will over-weight far-dated round-number strikes exactly the way the
  // all-expiry walls did. Writing it in core upgrades this line with no edit here.
  const cols = sql`
    cycle_time,
    spot,
    ...
    net_gamma_at_spot,
    coalesce(near_term -> 'strikes', strikes) as strikes`;
  ...
  return {
    ...
    strikes: toStrikeGex(r["strikes"]),
  };
}

/**
 * flip|callWall|putWall|netGamma|unixMillis|k:gex,k:gex,… — the format gamma-levels.pine parses.
 *
 * The strike field is APPENDED, never inserted. The study's parser keys on position and only
 * requires three fields, so a blob written before this field existed still reads correctly,
 * and an old study reading a new blob simply ignores the tail. With no zones the field is
 * omitted entirely rather than emitted empty — a trailing "|" would parse as a 6th field
 * holding one unparseable pair.
 */
function toBlob(s: Snapshot): string {
  const f = (v: number | null) => (v === null ? "0" : String(Math.round(v * 100) / 100));
  const head = [f(s.flip), f(s.callWall), f(s.putWall), f(s.netGamma),
    String(s.cycleTime.getTime())];
  const zones = pickZones(s.strikes, s.spot);
  if (zones.length === 0) return head.join("|");
  return [...head, zones.map((z) => `${f(z.k)}:${f(z.gex)}`).join(",")].join("|");
}
...
async function pushOnce(sql: SQL, dry: boolean, lastGood: boolean): Promise<void> {
  ...
  const nZones = pickZones(snap.strikes, snap.spot).length;
  console.warn(
    `[${at}] flip=${snap.flip} call=${snap.callWall} put=${snap.putWall} ` +
      `netΓ=${snap.netGamma.toFixed(2)} zones=${nZones}/${snap.strikes.length} age=${ageMin}m`,
  );
