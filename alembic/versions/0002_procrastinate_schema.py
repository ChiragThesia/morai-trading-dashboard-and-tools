...
Procrastinate 3.9.0's own `schema.sql` (from `procrastinate/sql/schema.sql` inside
the installed package), taken verbatim via `op.execute()` (D-13). Procrastinate
ships this SQL as a plain file meant for `psql`, with no Alembic integration of its
own and no tracked migration state outside its own package. Wrapping it here,
chained onto the 01-03 baseline, keeps Alembic the single migration system of
record: `alembic upgrade head` brings a fresh database to a state that includes
Procrastinate's tables, and `procrastinate schema --apply` is never run as a
deploy step.

A future Procrastinate version bump that changes this schema supersedes this
revision with a new one -- do not edit the SQL below in place, since a deployed
database will already have this exact schema.sql applied.
"""

# ruff: noqa: E501
# The line above disables the line-length rule for this whole file. The block below
# is vendor SQL taken verbatim (task instruction: do not reformat, reorder, or clean
# up); several of its lines exceed 88 columns as shipped by Procrastinate 3.9.0.
...
# Procrastinate 3.9.0's `procrastinate/sql/schema.sql`, taken verbatim -- do not
# reformat, reorder, or clean up. Read alongside the installed package if this
...
def downgrade() -> None:
    # Reverse dependency order: events/periodic_defers/jobs reference each other
    # via FK, jobs references workers. Dropping a table drops its own indexes and
    # triggers with it, so those need no separate DROP here. Functions have no
    # remaining dependents once the tables are gone. The `plpgsql` extension
    # created in upgrade() is left in place -- dropping it is a Postgres-wide,
    # not Procrastinate-scoped, action and risks cascading into objects this
    ...
    op.execute(
        ...
        "procrastinate_trigger_function_status_events_insert_v1 CASCADE"
    )
    op.execute(
        "DROP FUNCTION IF EXISTS "
        "procrastinate_trigger_function_status_events_update_v1 CASCADE"
    )
    op.execute(
        "DROP FUNCTION IF EXISTS "
        "procrastinate_trigger_function_scheduled_events_v1 CASCADE"
    )
    op.execute(
        "DROP FUNCTION IF EXISTS "
        "procrastinate_trigger_abort_requested_events_procedure_v1 CASCADE"
    )
    op.execute(
