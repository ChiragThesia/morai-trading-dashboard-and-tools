...
import { scheduledDedupeKey, rebuildDedupeKey } from "../domain/dedupe-key.ts";
 ...
 *
 * rebuild-journal: payload.calendarId is the natural scope (one rebuild per calendar at a time).
...
function buildDedupeKey(
  ...
  if (name === "rebuild-journal") {
    ...
    if (typeof calendarId === "string" && calendarId.length > 0) {
      return rebuildDedupeKey(calendarId);
    }
