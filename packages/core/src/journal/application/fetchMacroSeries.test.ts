 ...
 *
 * Tests verify:
 *   1. all 9 fetches succeed → every row persisted, driver returns ok(void)
 *   2. 2 series fail (fetch err) → 7 successes STILL persisted, driver returns err
...
describe("makeFetchMacroSeries", () => {
  it("persists every row and returns ok(void) when all 9 series succeed", async () => {
  ...
  it("persists the 7 successes and returns err naming both failed series when 2 fetches fail", async () => {
    const persisted: Array<MacroObservationRow> = [];
  ...
  it("counts a persist failure in the finish without blocking attempts on other series", async () => {
    ...
    // 8 of 9 series persisted successfully — SOFR's persist failed but the others still attempted
