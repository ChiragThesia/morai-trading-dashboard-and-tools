 ...
 *
 * Tests verify:
 *   1. all 10 fetches succeed → every row persisted, driver returns ok(void)
 *   2. 2 series fail (fetch err) → 8 successes STILL persisted, driver returns err
...
describe("makeFetchMacroSeries", () => {
  ...
  it("persists the 8 successes and returns err naming both failed series when 2 fetches fail", async () => {
    const persisted: Array<MacroObservationRow> = [];
