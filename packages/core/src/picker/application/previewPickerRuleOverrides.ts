...
import { debitFitFraction, WEIGHT_BACK_EVENT_BONUS } from "../domain/rules.ts";
...
function rescoreCandidate(
  ...
  // Event-bucket candidates carry STANDARD weights in their stored breakdown and their
  // stored score is exactly Σ(w×c)/100 + WEIGHT_BACK_EVENT_BONUS × bonus — the engine
  // normalizes by weight sum, so the event registry's ×0.9 scale cancels out (verified
  // against all 8 live event candidates, 2026-07-10). Preview therefore re-weights with
  // the plain staged weights and re-adds the stored bonus term; scaling the weights here
  // faked a uniform −5 delta on every event candidate (live UAT regression).
  ...
  const rawScore =
    breakdown.reduce((sum, entry) => sum + (weights[entry.criterion] * entry.contribution) / 100, 0) + bonusTerm;
