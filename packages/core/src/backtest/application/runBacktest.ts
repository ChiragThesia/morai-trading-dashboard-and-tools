...
const CAVEATS: ReadonlyArray<string> = [
  ...
  "residual BSM bias, migration 0028: ingest wrote contracts.root from the REQUESTED chain label and contracts.expiration off local date getters, so 1,294 contracts were solved with T short by a full day and their bsm_iv is biased HIGH. 0028 repaired both columns and deliberately did NOT re-solve the 1,576,510 observation rows underneath — blanking them was ~41 days of BSM drain, and every consumer filters bsm_iv IS NOT NULL, so a blanked row VANISHES rather than reads biased. Observations from 2026-07-06 to 2026-07-27 therefore carry a tenor-scaled IV bias, roughly 0.5/DTE in relative terms: 16.5 vol points at 1 DTE, 4.2 at 2, 1.2 at 5, 0.92 at 7, 0.43 at 15, under 0.34 beyond 20. It reaches the picker-cohort and hypothetical-entry chain reads and the daily forward walk that picks the exit, and it reaches BT-03 by a second route — calendar_snapshots.front_iv/back_iv are write-time copies of the same bsm_iv. The stored BSM greeks are read but reach no report field (MarketContext carries no greek fields; scoring re-derives delta from IV). Disclosed, not filtered: 0028 overwrote root and expiration in place and the pre-image is gone (contracts has no created_at, schwab_symbol is NULL on all 26,347 rows, leg_observations has no bsm_solved_at, and backtest_runs held no pre-repair report to diff), so the affected contracts are no longer individually identifiable. The corruption itself is closed — every contract now agrees with its own OCC symbol. Excluding the window is not the alternative: it would leave n=1 of 946 cohorts.",
  "0028 expiration-repair artifact (BT-02): the same migration also repaired contracts.expiration, which the backtest chain read joins at READ time. So the leakage oracle compares candidates replayed off REPAIRED expirations against picker_snapshot rows written with CORRUPT ones, and a membership or score mismatch dated 2026-07-06 to 2026-07-27 may be a repair artifact rather than leakage. Measured lower bound: 30 stored candidate legs (11 front, 19 back) across 25 of 488 candidate-bearing cohorts name expirations that do not exist in the repaired contracts table, two of them Sundays (2026-08-02, 2026-08-09). Only a LOWER bound — a corrupt date is detectable only when the day before the true expiry is not itself a listed expiry — so the true rate is unquantified. BT-02 still reads: its mismatch count over that window is an upper bound on leakage, not a measurement of it.",
];
