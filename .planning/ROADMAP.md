...
- [ ] 03-02-PLAN.md — Wave 2. Migration 0008 (`positions`, `legs`, `events`), the netted-ROLL `CHECK` proved by raw SQL on a connection that bypasses the application, and the events write path that keeps a compound event's two amounts split
...
| 3. Envelope Encryption and the Schema Contract | 1/7 | In Progress|  |
