...
- [x] 29-06-PLAN.md — Regime seam + resolveRegimeRuleConfig (four band thresholds)
...
- [x] 29-13-PLAN.md — Server surface: GET/PUT /api/settings/rules + MCP tools + engine-computed defaults
...
- [x] 30-02-PLAN.md — Thread the domain into Analyzer + Overview payoff charts; combined-book non-regression (D-01)
- [x] 30-04-PLAN.md — makeAnalyzeAdHocCalendarUseCase: score one ad-hoc PUT calendar with engine parity, reused gate/sizing (D-02)
...
- [x] 32-03-PLAN.md — Exit preview use-case (live evaluateExit current-vs-staged, structural no-persist)
...
- [x] 36-01-PLAN.md — Analyzer switch + useAnalyzerModel + MobileChartControls extraction + desktop-test migration (wave 1)
- [x] 36-02-PLAN.md — AnalyzerMobile tree: paste block, candidates fold, MobileScorecard, chart block, disclosures (wave 2)
...
- [x] 37-01-PLAN.md — docs-first decision + migration 0024 reauth_nonces (wave 1)
- [x] 37-02-PLAN.md — contracts + core reauth port/use-cases + sidecar adapter (wave 1)
- [x] 37-03-PLAN.md — sidecar re-init plumbing + refresh_issued_at writer + config secrets (wave 1)
- [x] 37-04-PLAN.md — sidecar admin endpoints: /start + /exchange (nonce, freshness gate, reinit) (wave 2)
- [x] 37-05-PLAN.md — server JWT-gated proxy routes + config + wiring (MCP scoped out) (wave 2)
- [x] 37-06-PLAN.md — web Reconnect wizard + callback capture/strip + banner (wave 2)
- [x] 37-07-PLAN.md — integration gate + runbook UI path + deploy/env + live re-auth UAT (wave 3)
...
- [x] 38-03-PLAN.md — server spot+indices fan-out (on-change ~1/sec) + sidecar-sse dispatch + main.ts (wave 2)
- [x] 38-04-PLAN.md — web hook + model seam: useLiveStream liveSpot/liveIndices + live-aware model spot (wave 2)
...
- [x] 38-07-PLAN.md — integration gate + Railway/Vercel deploy + live RTH UAT (wave 5)
...
- [x] 39-02-PLAN.md — rates block as gauges (neutral 4 + banded 2, RATE_BANDS gate-blind) + regime/rate 4-part teaching tooltips (wave 2)
- [x] 39-03-PLAN.md — COT rows as neutral direction-tinted gauges + typography + 4-part COT tooltips (wave 2)
- [x] 39-04-PLAN.md — integration gate (suite/typechecks/lint + cross-cutting law greps) + deploy (Vercel) + desktop UAT on morai.wtf (wave 3)
...
- [x] 42-02-PLAN.md — CandidateTable → thin DataTable wrapper, public API byte-stable, Analyzer desktop+mobile untouched (wave 2)
- [x] 42-03-PLAN.md — Overview PositionsTable → DataTable columns; detail + Net-total rows via caller slots (wave 2)
- [x] 42-04-PLAN.md — single Button: migrate dialog/Login/RebuildButton to system/Button, token sweep, delete ui/button.tsx (wave 1)
- [x] 42-05-PLAN.md — design-system.md + TOPIC-MAP + rule link; phase gate (full suite, tsc baseline, dual-viewport visual parity) (wave 3)
