...
- [x] **GW-03**: The TS `refresh-tokens` job is retired and the sidecar is the only token refresher — exactly one process ever writes `broker_tokens` (no dual-refresher rotating-token race).
...
- [x] **STRM-02**: The sidecar streams ACCT_ACTIVITY fill events.
- [x] **STRM-03**: `apps/server` fans the sidecar's single stream out to N browser clients over an authed `GET /api/stream`, with the Supabase JWT verified at the server edge.
- [x] **STRM-04**: Stream data is display-only — no per-tick persistence; `sync-transactions` (REST) remains the authoritative fill source.
...
- [x] **AUTH-06**: A one-click/operator re-auth flow (manual-flow → `token_write` to Postgres) restores Schwab auth without a redeploy.
...
| GW-03 | Phase 11 | Complete |
| GW-04 | Phase 11 | Complete |
| GW-05 | Phase 11 | Complete |
| JRNL-02 | Phase 11 | Complete |
| STRM-01 | Phase 12 | Complete |
| STRM-02 | Phase 12 | Complete |
| STRM-03 | Phase 12 | Complete |
| STRM-04 | Phase 12 | Complete |
