...
| 05-01-03 | 01 | 1 | JOB-01/JRNL-01 | T-05-01 | All 9 owned Wave-0 stubs exist + RED on assertions (not import errors) | unit | shell loop asserts 9 stub files exist, then `cd packages/core && bun run test src/journal/domain/fill-pairing.test.ts` FAILs on assertion (not module-resolution) | ❌ W0 | ⬜ pending |
...
| 05-05-02 | 05 | 4 | JOB-02 | T-05-11/12 | No RTH gate; one-app failure isolated; per-app GET /api/status flag (broker_tokens.last_refresh_error, no new table); no token logs | unit | `cd apps/worker && bun run test src/handlers/refresh-tokens.test.ts && rg -q "last_refresh_error" packages/adapters/src/postgres/migrations/0005_broker_tokens_refresh_error.sql && rg -q "lastRefreshError" packages/contracts/src/status.ts` | ❌ W0 | ⬜ pending |
...
| 05-08-02 | 08 | 6 | JRNL-01/MCP-02 | T-05-21/22/24 | trigger_job HTTP+MCP share schema; auth-guarded | unit | `cd apps/server && bun run test src/adapters/http/jobs.routes.test.ts` | ❌ W0 | ⬜ pending |
...
schema — feedback latency stays within bounds. The 05-05 `broker_tokens.last_refresh_error`
status surface is proven against the in-memory broker-tokens twin (no live-DB gate needed); its
additive migration `0005_broker_tokens_refresh_error.sql` is verified by file grep in 05-05-02.
...
enqueueJob / schedule / token-freshness / jobs.routes RED stubs are created within their own
consuming plans' RED steps (05-04 / 05-05 / 05-08), NOT in 05-01. 05-01 Task 3 owns exactly the
9 stubs in the list below marked "05-01-owned", and its `<automated>` verify asserts all 9 exist
and that fill-pairing.test fails RED for the right reason (assertion, not module resolution).

- [ ] `packages/core/src/journal/domain/fill-pairing.test.ts` (05-01-owned) — classifyFill/aggregatePartialFills/computePnl/detectRoll/hashFillIds (example + fast-check) — driven GREEN by 05-03
- [ ] `packages/core/src/journal/domain/dedupe-key.test.ts` (05-04-owned) — scheduledDedupeKey/rebuildDedupeKey — driven GREEN by 05-04
- [ ] `packages/core/src/journal/application/enqueueJob.test.ts` (05-04-owned) — enqueue idempotency — 05-04
- [ ] `packages/core/src/journal/application/syncFills.test.ts` (05-01-owned) — OPEN/CLOSE/ROLL, orphan parking, idempotency — driven GREEN by 05-07
- [ ] `packages/core/src/journal/application/rebuildJournal.test.ts` (05-01-owned) — delete-then-reinsert + SC5 reconciliation — driven GREEN by 05-08
- [ ] `packages/core/src/brokerage/application/refreshTokens.test.ts` (05-01-owned) — per-app independence + expiry warning — driven GREEN by 05-05
- [ ] `packages/core/src/brokerage/domain/token-freshness.test.ts` (05-05-owned) — isNearExpiry — 05-05
- [ ] `packages/adapters/src/__contract__/calendar-events.contract.ts` + `__contract__/orphan-fills.contract.ts` (05-01-owned) — shared contract modules — filled in by 05-07
- [ ] `packages/adapters/src/postgres/repos/leg-observations.bsm-drain.contract.test.ts` (05-06-owned) — SC3 drain (testcontainers) — 05-06
- [ ] `apps/worker/src/handlers/sync-fills.test.ts` (05-01-owned) — RTH gate + delegation — 05-07
- [ ] `apps/worker/src/handlers/refresh-tokens.test.ts` (05-01-owned) — no RTH gate + one-app isolation + per-app /api/status lastRefreshError flag — 05-05
- [ ] `apps/worker/src/handlers/rebuild-journal.test.ts` (05-01-owned) — on-demand handler — 05-08
- [ ] `apps/worker/src/schedule.test.ts` (05-04-owned) — 7 createQueue / 5 schedule (snapshot + rebuild cronless) — 05-04
- [ ] `apps/server/src/adapters/http/jobs.routes.test.ts` (05-08-owned) — trigger_job HTTP — 05-08
- [ ] Shared fixtures: testcontainers Postgres harness (reuse calendar-snapshots.contract.test.ts pattern); msw Schwab refresh handler (reuse Phase 4); in-memory JobQueue twin; in-memory broker-tokens twin (carries last_refresh_error for the 05-05 status assertion)
