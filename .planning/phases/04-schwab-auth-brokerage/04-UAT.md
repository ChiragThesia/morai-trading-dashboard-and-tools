...
result: pass
note: "Was issue (major). Fixed: runDoctorCommand now wires the real makeRefreshTokenUseCase + stored token (commit 3f1c699), dummy '__doctor_probe__' removed. LIVE-VALIDATED: `auth doctor` → 'live refresh: OK — token rotated successfully'."
...
result: pass
note: "Was issue (major). Fixed: adapter now sends strikeCount/range/fromDate/toDate scoping (commits 15bbf44/af1c5bf). LIVE-VALIDATED: scoped $SPX → HTTP 200, real chain, underlying 7500.58, 3582 contracts (was 20k+ → 502), 33 expirations in a 90-day window."
