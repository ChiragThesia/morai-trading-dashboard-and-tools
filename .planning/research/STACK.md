...
**Project:** Morai Journal — multi-user Python trading-journal backend
**Researched:** 2026-08-29
**Confidence:** MEDIUM-HIGH (version numbers verified live against PyPI JSON API and official docs; a few judgment calls — KMS-vs-env-var, session-store choice — are architectural opinion, marked LOW/MEDIUM below, not vendor fact)

Scope note: this file is stack only. Domain rules (ledger correctness, the 45 non-negotiables,
metric selection) are answered elsewhere in this repo and are not re-derived here.
...
- **No-`Any` configuration for Pydantic under strict mypy:** Pydantic's own mypy plugin synthesizes an `__init__` typed with `Any` for every field unless configured otherwise. Fix in `pyproject.toml`:
  ...
  ```
  `init_typed = true` is the specific flag that replaces the synthesized `Any` parameter types with the model's real field types, which is what lets `--disallow-any-explicit` pass on Pydantic models at all. **Confidence: MEDIUM** (official Pydantic docs, but exact flag interaction is version-sensitive — re-verify against whatever mypy/Pydantic patch versions land in the lockfile).
  Under basedpyright, no plugin is needed — basedpyright understands Pydantic's `__init__` synthesis natively as of recent releases; if it doesn't in the pinned version, that's the one place a narrowly-scoped `# pyright: ignore[reportUnknownArgumentType]` on the synthesized call site is more honest than fighting the plugin, since this project already forbids bare `# type: ignore` but a *ruled* ignore names the exact diagnostic it suppresses.
...
```bash
# Core
pip install fastapi "pydantic>=2.13" "sqlalchemy[asyncpg]>=2.0" alembic procrastinate \
    "cryptography>=50" argon2-cffi hypercorn schwab-py

# Dev / test / type-check
pip install pytest pytest-asyncio hypothesis basedpyright mypy ruff
...
**If the Schwab OAuth callback needs to run behind Railway's proxy:**
...
**If a user needs to re-authorize after the 7-day refresh expiry:**
...
---
*Stack research for: multi-user Python trading-journal backend*
*Researched: 2026-08-29*
